...
var _ = Describe("Rest", func() {
	...
	Describe("Verify()", func() {
		It("Verifies entered names", func() {
			...
			Expect(bad.CurationLevel).To(Equal(vlib.NotCurated))
			...
			Expect(binom.BestResult.MatchType).To(Equal(vlib.Exact))
			...
			Expect(acceptFilter.BestResult.MatchType).To(Equal(vlib.Exact))
			...
			Expect(partial.MatchType).To(Equal(vlib.PartialExact))
